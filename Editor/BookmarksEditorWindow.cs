...
namespace UnityEditor.SceneViewBookmarks
{
    public class BookmarkEditorWindow : EditorWindow
    {
        ...
        private void RefreshUI()
        {
            ...
            if (SceneBookmarks.Count == 0)
                return;

            for (int i = 0; i < SceneBookmarks.Count; i++)
            {
                var index = i;
                ...
                Button loadButton = new Button(() => SceneBookmarks.current[index].Load(SceneView.lastActiveSceneView)) { name = "LOAD", text = "LOAD" };
                loadButton.AddToClassList("svbm-cell-button");
                bookmarkElement.Add(loadButton);

                Button updateButton = new Button(() => SceneBookmarks.current[index].Save(SceneView.lastActiveSceneView)) { name = "UPDATE", text = "UPDATE" };
                updateButton.AddToClassList("svbm-cell-button");
                bookmarkElement.Add(updateButton);

                Button deleteButton = new Button(() =>
                {
                    SceneBookmarks.RemoveAt(index);
                    RefreshUI();
                ...
                nameField.value = SceneBookmarks.current[index].name;
                nameField.AddToClassList("svbm-cell-text");
                nameField.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].name = x.newValue);
                bookmarkElement.Add(nameField);

                MaskField overridesMask = new MaskField(Enum.GetNames(typeof(Viewpoint.Overrides)).ToList(), (int)SceneBookmarks.current[index].overrides) { tooltip = "Set this bookmark's Overrides." };
                overridesMask.AddToClassList("svbm-cell-mask");
                overridesMask.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].overrides = (Viewpoint.Overrides)x.newValue);
                ...
                orthoToggle.value = SceneBookmarks.current[index].settings.ortho;
                orthoToggle.AddToClassList("svbm-cell-checkbox");
                orthoToggle.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].settings.ortho = x.newValue);
                bookmarkElement.Add(orthoToggle);

                Toggle is2dToggle = new Toggle("") { tooltip = "Enable to set this bookmark's SceneView to 2D Mode." };
                is2dToggle.value = SceneBookmarks.current[index].settings.is2D;
                is2dToggle.AddToClassList("svbm-cell-checkbox");
                is2dToggle.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].settings.is2D = x.newValue);
                bookmarkElement.Add(is2dToggle);

                FloatField fovField = new FloatField() { tooltip = "Set this bookmark's Camera Field of View." };
                fovField.value = SceneBookmarks.current[index].settings.fov;
                fovField.AddToClassList("svbm-cell-float");
                fovField.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].settings.fov = Mathf.Clamp(x.newValue, 4, 120));
                bookmarkElement.Add(fovField);

                EnumField cameraModeMask = new EnumField("", SceneBookmarks.current[index].settings.mode.drawMode) { tooltip = "Set this bookmark's Camera Shading Mode." };
                ...
                cameraModeMask.RegisterValueChangedCallback((x) => {
                    try
                    {
                        SceneBookmarks.current[index].settings.mode = SceneView.GetBuiltinCameraMode((DrawCameraMode)x.newValue);
                    }
                    catch
                    {
                ...
                lightingToggle.value = SceneBookmarks.current[index].settings.sceneLighting;
                lightingToggle.AddToClassList("svbm-cell-checkbox");
                lightingToggle.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].settings.sceneLighting = x.newValue);
                bookmarkElement.Add(lightingToggle);

                MaskField viewStatesMask = new MaskField(SceneViewStatesLabels, (int)SceneBookmarks.current[index].settings.sceneViewState.GetFlags()) { tooltip = "Set this bookmark's SceneView States." };
                viewStatesMask.AddToClassList("svbm-cell-mask");
                viewStatesMask.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].settings.sceneViewState.SetFlags((SceneViewExtensions.SceneViewStateFlags)x.newValue));
                bookmarkElement.Add(viewStatesMask);

                Toggle gridToggle = new Toggle("") { tooltip = "Enable this bookmark's SceneView Grid." };
                gridToggle.value = SceneBookmarks.current[index].settings.showGrid;
                gridToggle.AddToClassList("svbm-cell-checkbox");
                gridToggle.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].settings.showGrid = x.newValue);
                bookmarkElement.Add(gridToggle);

                Toggle gizmosToggle = new Toggle("") { tooltip = "Enable this bookmark's SceneView Gizmos." };
                gizmosToggle.value = SceneBookmarks.current[index].settings.drawGizmos;
                gizmosToggle.AddToClassList("svbm-cell-checkbox");
                gizmosToggle.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].settings.drawGizmos = x.newValue);
                bookmarkElement.Add(gizmosToggle);

                LayerMaskField visibleLayerMask = new LayerMaskField("", SceneBookmarks.current[index].visibleLayers) { tooltip = "Set this bookmark's Visible Layers." };
                visibleLayerMask.AddToClassList("svbm-cell-mask");
                visibleLayerMask.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].visibleLayers = x.newValue);
                bookmarkElement.Add(visibleLayerMask);

                LayerMaskField lockedLayerMask = new LayerMaskField("", SceneBookmarks.current[index].lockedLayers) { tooltip = "Set this bookmark's Locked Layers." };
                lockedLayerMask.AddToClassList("svbm-cell-mask");
                lockedLayerMask.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].lockedLayers = x.newValue);
                bookmarkElement.Add(lockedLayerMask);

                EnumField shortcutMask = new EnumField("", SceneBookmarks.current[index].shortcut) { tooltip = "Set this bookmark's shortcut\nBeware this will override any other function the shortcut is associated with." };
                shortcutMask.AddToClassList("svbm-cell-mask");
                shortcutMask.RegisterValueChangedCallback((x) => SceneBookmarks.current[index].shortcut = (KeyCode)x.newValue);
                ...
                Button moveUpButton = new Button(() =>
                {
                    ...
                    var item = SceneBookmarks.current[index];
                    SceneBookmarks.RemoveAt(index);

                    SceneBookmarks.Insert(index - 1, item);
                    RefreshUI();
                })
                { name = "Up", text = "Up" };
                ...
                {
                    if (index >= SceneBookmarks.Count - 1)
                        return;
